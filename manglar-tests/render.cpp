...
#ifdef BELA_LIBPD_DISABLE_DHT
#undef  BELA_LIBPD_DHT
#endif // BELA_LIBPD_DISABLE_DHT
...
#ifdef BELA_LIBPD_DHT
#include "DHT.h"
DHT* gDht = nullptr;
#endif // BELA_LIBPD_DHT
...
#ifdef BELA_LIBPD_DHT
	if(0 == strcmp(source, "bela_setDht")) {
		int channel = value;
		gDht = new DHT(channel, DHT11);
		rt_printf("Enabling DHT11 sensor on channel %d. Will send to [r bela_dht]\n", channel);
		return;
	}
#endif // BELA_LIBPD_DHT
}
...
void render(BelaContext *context, void *userData)
{
	...
	{
		static int count = 0;
		static bool stateWasDisabled = true;
		if(count > 2 * context->digitalSampleRate)
			count = 0;
		if(0 == count)
			gDht->state = DHT::kShouldStart;
		count += context->digitalFrames;
		gDht->process(context);
		bool stateIsDisabled = DHT::kDisabled == gDht->state;
		if(stateIsDisabled && !stateWasDisabled)
		{
			// result is ready
			libpd_start_message(2);
			libpd_add_float(gDht->readTemperature());
			libpd_add_float(gDht->readHumidity());
			libpd_finish_list("bela_dht");
		}
		stateWasDisabled = stateIsDisabled;
	}
#endif // BELA_LIBPD_DHT
#ifdef BELA_LIBPD_GUI
